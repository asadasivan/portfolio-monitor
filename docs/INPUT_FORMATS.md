...
Reports convert INR holdings into the configured `base_currency` for totals, allocation percentages, reconciliation, and risk calculations. Configure conversion rates in `currency_conversion.rates_to_base`. See [Reports](REPORTS.md) for output-currency settings such as `Value (INR)` and live FX refresh behavior.
